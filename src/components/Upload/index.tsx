...
import { useCheckQueueAndFetchImage } from "@/hooks/useCheckQueueAndFetchImage";

const Upload = () => {
  const [statusMessage, setStatusMessage] = useState<string | null>(null);
  const [isError, setIsError] = useState<boolean>(false);
  const [isDownloading, setIsDownloading] = useState<boolean>(false);
  const { processedImageUrl, checkQueueAndFetchImage } = useCheckQueueAndFetchImage({setIsError, setStatusMessage, setIsDownloading});
  ...
  const handleFileUpload = async (files: Blob) => {
    ...
    reader.onloadend = async () => {
      ...
      if (data && data.array_map_queue && data.array_map_queue.length > 0) {
        ...
        checkQueueAndFetchImage(data.array_map_queue[0].id_queue);
      }
    };
        ...
        <div className="text-center">
          ...
          <div className="flex justify-center items-center mb-4">
            <div className="container mx-auto">
              {processedImageUrl ? (
                <Render imageUrl={processedImageUrl} isDownloading={isDownloading} />
              ) : uploadedImageUrl ? (
                <Render imageUrl={uploadedImageUrl} isDownloading={isDownloading} />
              ) : (
